...
- **KPI strip reduced from 8 to 4 priority metrics** (Revenue, Net income, Operating cash, Gross margin) per user feedback. Dropped: Operating expenses, AR aging >60d, AP outstanding, Headcount cost. These are detail/secondary metrics better suited to drill-down tabs (AR/AP, Budget vs actual, Headcount).
- **KPI cards now flex-grow** (`flex: 1 1 220px` instead of `flex: 0 0 220px`) so the 4 remaining cards fill the available horizontal space more evenly on desktop.

#### Surfaces fully implemented (not deviated)
- Three-breakpoint responsive (≤900 / ≤640 / ≤400 px) per spec.
- 4-card KPI strip with composed sparklines, delta Tag, `role="list"/"listitem"`, aria-labels.
